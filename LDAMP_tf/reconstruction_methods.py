...
def reconstruction_method(dset,sensing_method,specifics):
    method = LDAMP_wrapper(specifics)
    return method
...
def CountParameters():
    ...
    for variable in tf.trainable_variables():
        ...
        for dim in shape:
            variable_parameters *= dim.value #TODO # originaly dim.value instead of dim
...
class LDAMP_wrapper():
    def __init__(self,specifics):
        ...
        self.is_complex = False  # Just the default
    ...
    def run(self):
        ...
        if(self.DenoiserbyDenoiser):
            ...
            train_images = np.load('./Data/TrainingData_patch' + str(height_img) + '.npy')
            train_images = train_images[range(n_Train_Images), 0, :, :]
            assert (len(train_images) >= n_Train_Images), "Requested too much training data"

            val_images = np.load('./Data/ValidationData_patch' + str(height_img) + '.npy')
