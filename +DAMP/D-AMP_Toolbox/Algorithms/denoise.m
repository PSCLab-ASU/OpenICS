...
switch denoiser
    ...
    case 'BM3D'
        % modified to use conventions for BM3D release v3.0.7
        output=BM3D(noisy,sigma_hat,'np');
    case 'fast-BM3D'
        % modified to use conventions for BM3D release v3.0.7
        noisy=real(noisy);
        output=BM3D(noisy,sigma_hat,'lc');
    case 'CBM3D'
        % modified to use conventions for BM3D release v3.0.7
        output=CBM3D(noisy,sigma_hat,'np');
    % removed BM3D-SAPCA case, since no longer exists in new installation
    % disabled DnCNN case, since technically learned denoiser
    case 'DnCNN'
