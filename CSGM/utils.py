...
def generate_dataset(dataset,input_channel,input_width,input_height,stage):
    ...
    return model_input #for CSGM, this returns the appropriate python module that the algorithm will use, based on which dataset is needed

def generate_dataset_(dataset,input_channel,input_width,input_height,stage, specifics):
...
def image_matrix(images, est_images, view_image, hparams, alg_labels=True):
    ...
    for model_type in hparams.model_types:
        outer_counter += 1
        for image in list(est_images[model_type].values()):
            inner_counter += 1
            ax = fig.add_subplot(figure_height, 1, outer_counter, frameon=False)
            ax.get_xaxis().set_visible(False)
            ax.get_yaxis().set_ticks([])
...
def get_opt_reinit_op(opt, var_list, global_step):
    ...
    if isinstance(opt, tf.train.AdamOptimizer):
        ...
        opt_slots.extend(beta_powers)#[opt._beta1_power, opt._beta2_power])  #pylint: disable = W0212
