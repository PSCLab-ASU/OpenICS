...
if __MOSEK:
    ...
    def l1regls_mosek(A, b):
        ...
        task.putclist(list(range(2*n)), list(-2*A.T*b) + n*[1.0])  # setup linear objective
    ...
    def l1regls_mosek2(A, b):
        ...
        # input quadratic objective
        task.putqobj(list(range(2*n,2*n+m)), list(range(2*n,2*n+m)), m*[2.0])

        task.putclist(list(range(2*n+m)), n*[0.0] + n*[1.0] + m*[0.0])  # setup linear objective
