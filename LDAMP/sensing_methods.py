...
class random_sensing(nn.Module):
    ...
    def forward(self, x):
        r = self.s1(x)
        noise = self.sigma * torch.rand(r.shape).cuda()
        r = r + noise
        return r
