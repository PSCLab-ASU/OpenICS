...
for n_DAMP_layers in range(start_layer,max_n_DAMP_layers+1,1):
    ...
    x_true = torch.tensor([n, BATCH_SIZE],dtype = torch.float32)#tf.placeholder(tf.float32, [n, BATCH_SIZE])
    ...
    if loss_func=='SURE':
    ...
    elif loss_func=='GSURE': #NOT IMPLEMENTED YET
        assert alg == 'DAMP', "Only LDAMP currently supports training with GSURE"
        
        temp0 = torch.matmul(A_val_tf, torch.transpose(A_val_tf)) #temp0=torch.matmul(A_val_tf,A_val_tf,transpose_b=True)
        temp1 = torch.inverse(temp0) #temp1=tf.matrix_inverse(temp0)
        pinv_A = torch.matmul(torch.transpose(A_val_tf),temp1) #pinv_A=tf.matmul(A_val_tf,temp1,transpose_a=True)
        P = torch.matmul(pinv_A, A_val_tf) #P=tf.matmul(pinv_A,A_val_tf)
        ...
        if sigma_w==0.:#Not sure if TF is smart enough to avoid computing MCdiv when it doesn't have to
            ...
            epsilon = torch.max(.001 * tf.reduce_max(Aty_tf, axis=0), .00001)
        ...
        cost = torch.sum((x_true - x_hat)**2)/2 *1./nfp#tf.nn.l2_loss(x_true - x_hat) * 1. / nfp
