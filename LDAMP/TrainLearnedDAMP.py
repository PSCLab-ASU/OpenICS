...
max_Epoch_Fails = (
    3
)  # How many training epochs to run without improvement in the validation error
...
for n_DAMP_layers in range(start_layer, max_n_DAMP_layers + 1, 1):
    ...
    LDAMP.SetNetworkParams(
        new_height_img=height_img,
        new_width_img=width_img,
        new_channel_img=channel_img,
        new_filter_height=filter_height,
        new_filter_width=filter_width,
        new_num_filters=num_filters,
        new_n_DnCNN_layers=n_DnCNN_layers,
        new_n_DAMP_layers=n_DAMP_layers,
    ...
    # tf Graph input
    # training_tf = torch.tensor(dtype=torch.bool) --dont know shape for this yet   #tf.placeholder(tf.bool, name='training')
    x_true = torch.tensor(
        [n, BATCH_SIZE], dtype=torch.float32
    )  # tf.placeholder(tf.float32, [n, BATCH_SIZE])
    ...
    [A_handle, At_handle, A_val, A_val_tf] = LDAMP.GenerateMeasurementOperators(
        measurement_mode
    )
    ...
    if alg == "DAMP":
        ...
        (x_hat, MSE_history, NMSE_history, PSNR_history) = LDAMP.LDIT(
            y_measured,
            A_handle,
            At_handle,
            A_val_tf,
    ...
    if loss_func == "SURE":
        ...
        (x_hat, _, _, _, _, _, _) = LDAMP.LDAMP_Aty(
            Aty_tf,
            A_handle,
            At_handle,
            A_val_tf,
        ...
        if (
            sigma_w == 0.0
        ):  # Not sure if TF is smart enough to avoid computing MCdiv when it doesn't have to
            MCdiv = 0.0
        else:
            # Calculate MC divergence of P*LDAMP(Aty)
            epsilon = tf.maximum(0.001 * tf.reduce_max(Aty_tf, axis=0), 0.00001)
            eta = tf.random_normal(shape=Aty_tf.get_shape(), dtype=tf.float32)
            Aty_perturbed_tf = Aty_tf + tf.multiply(eta, epsilon)
            (x_hat_perturbed, _, _, _, _, _, _) = LDAMP.LDAMP_Aty(
                Aty_perturbed_tf,
                A_handle,
                At_handle,
                A_val_tf,
        ...
        cost = tf.nn.l2_loss(x_true - x_hat) * 1.0 / nfp
    ...
    if LayerbyLayer == True:
        ...
        for l in range(
            1, n_DnCNN_layers - 1
        ):  # Associate variance, means, beta, and gamma
            ...
            moving_variance = [v for v in tf.global_variables() if v.name == var_name][
                0
            ]
    ...
    for learning_rate in learning_rates:
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            if ResumeTraining or learning_rate != learning_rates[0]:
                ...
                saver_initvars_name_chckpt = (
                    LDAMP.GenLDAMPFilename(
                        alg, tie_weights, LayerbyLayer, loss_func=loss_func
                    )
                    + ".ckpt"
                )
                for iter in range(
                    n_layers_trained
                ):  # Create a dictionary with all the variables except those associated with the optimizer.
                    for l in range(0, n_DnCNN_layers):
                        saver_dict.update(
                            {
                                "Iter"
                    ...
                    for l in range(
                        1, n_DnCNN_layers - 1
                    ):  # Associate variance, means, and beta
                        gamma_name = "Iter" + str(iter) + "/l" + str(l) + "/BN/gamma:0"
                        beta_name = "Iter" + str(iter) + "/l" + str(l) + "/BN/beta:0"
                        var_name = (
                            "Iter" + str(iter) + "/l" + str(l) + "/BN/moving_variance:0"
                        )
                        mean_name = (
                            "Iter" + str(iter) + "/l" + str(l) + "/BN/moving_mean:0"
                        )
                        gamma = [
                        ...
                        saver_dict.update(
                            {"Iter" + str(iter) + "/l" + str(l) + "/BN/gamma": gamma}
                        )
                        saver_dict.update(
                            {"Iter" + str(iter) + "/l" + str(l) + "/BN/beta": beta}
                        )
                        saver_dict.update(
                            {
                                "Iter"
                                ...
                                + "/l"
                                + str(l)
                                + "/BN/moving_variance": moving_variance
                            }
                        )
                        saver_dict.update(
                            {
                ...
                if InitWeightsMethod == "layer_by_layer":
                    ...
                    saver_initvars_name_chckpt = (
                        LDAMP.GenLDAMPFilename(
                            alg, tie_weights, LayerbyLayer=True, loss_func=loss_func
                        )
                        + ".ckpt"
                    )
                    for iter in range(
                        ...
                        for l in range(
                            ...
                            gamma_name = (
                                "Iter" + str(iter) + "/l" + str(l) + "/BN/gamma:0"
                            )
                            beta_name = (
                                "Iter" + str(iter) + "/l" + str(l) + "/BN/beta:0"
                            )
                            var_name = (
                                "Iter"
                                + str(iter)
                ...
                if InitWeightsMethod == "denoiser":
                    ...
                    saver_initvars_name_chckpt = (
                        LDAMP.GenDnCNNFilename(300.0 / 255.0, 500.0 / 255.0) + ".ckpt"
                    )
                    iter = 0
                    for l in range(0, n_DnCNN_layers):
                        saver_dict.update(
                            {"l" + str(l) + "/w": theta[iter][0][l]}
                        )  # , "l" + str(l) + "/b": theta[iter][1][l]})
                    for l in range(
                        1, n_DnCNN_layers - 1
                    ):  # Associate variance, means, and beta
                        gamma_name = "Iter" + str(iter) + "/l" + str(l) + "/BN/gamma:0"
                    ...
                    for l in range(0, n_DnCNN_layers):
                        ...
                        )  # ,"Iter" + str(iter-1) + "/l" + str(l) + "/b": theta[iter][1][l]})
                    for l in range(
                        1, n_DnCNN_layers - 1
                    ):  # Associate variance, means, and beta
                        gamma_name = "Iter" + str(iter) + "/l" + str(l) + "/BN/gamma:0"
                        ...
                        saver_dict.update(
                            {"Iter" + str(iter - 1) + "/l" + str(l) + "/BN/beta": beta}
                        )
                        saver_dict.update(
                            {
                                "Iter"
            ...
            if __name__ == "__main__":
                ...
                rand_inds = np.random.choice(
                    len(val_images), n_Val_Images, replace=False
                )
                start_time = time.time()
                for offset in range(
                    0, n_Val_Images - BATCH_SIZE + 1, BATCH_SIZE
                ...
                if (
                    not LayerbyLayer
                ):  # For end-to-end training save the initial state so that LDAMP end-to-end doesn't diverge when using a high training rate
                ...
                for i in range(EPOCHS):
                    ...
                    rand_inds = np.random.choice(
                        len(train_images), n_Train_Images, replace=False
                    )
                    start_time = time.time()
                    for offset in range(
                        0, n_Train_Images - BATCH_SIZE + 1, BATCH_SIZE
                    ):  # Subtract batch size-1 to avoid errors when len(train_images) is not a multiple of the batch size
                        end = offset + BATCH_SIZE
                        ...
                        _, loss_val = sess.run(
                            [optimizer, cost],
                            feed_dict={
                                x_true: batch_x_train,
                                A_val_tf: A_val,
                                training_tf: True,
                            },
                        )  # Feed dict names should match with the placeholders
